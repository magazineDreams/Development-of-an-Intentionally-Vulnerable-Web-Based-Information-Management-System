...
1. SQL Injection
2. Files or Directories Accessible to External Parties CWE-552
...
app.secret_key = 'vulnerable_secret_key_12345'  # Weak secret key
app.config['SESSION_COOKIE_HTTPONLY'] = False  # Intentional: exposes cookie to JS (XSS demo)
...
def admin_dashboard():
    ...
    cursor.execute("SELECT id, first_name, last_name, email, role FROM users ORDER BY role, last_name")
    all_users = [{"id": r[0], "first_name": r[1], "last_name": r[2],
    ...
    """)
    all_courses = [{"id": r[0], "name": r[1], "code": r[2], "description": r[3],
    ...
    """)
    all_comments = [{"id": r[0], "content": r[1], "posted_at": r[2],
    ...
    """)
    all_announcements = [{"id": r[0], "title": r[1], "content": r[2],
                          "posted_at": r[3], "author": r[4], "subject_name": r[5]}
                         for r in cursor.fetchall()]

    conn.close()

    stats = {"students": total_students, "tutors": total_tutors,
             "courses": total_courses, "comments": total_comments,
             "announcements": total_announcements}

    return render_template("admin_dashboard.html", stats=stats,
                           all_users=all_users, all_courses=all_courses,
                           all_comments=all_comments, all_announcements=all_announcements)
...
def admin_edit_course(course_id):
    ...
    if name and code:
        conn = get_db()
        conn.execute("UPDATE courses SET name=?, code=?, description=? WHERE id=?",
...
def contact():
    ...
    if session.get('user_role') == 'tutor':
        ...
        # Fetch tutors with their subjects and office info
        cursor.execute("""
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user: 
                ...
                session["password"] = user[4]  # Intentional: MD5 hash exposed in session (XSS demo)
...
# student dashboartd handling 
    ...
    tutors_seen = set()
    tutors = []
    for c_id, c_name, c_code, s_id, s_name, s_code, t_id, t_first, t_last, t_email, t_office, t_hours in rows:
    ...
    return render_template("dashboard.html", courses=list(courses.values()), tutors=tutors, schedule=schedule)


@app.route("/profile/office", methods=["POST"]) # this is only for the office update
def update_office():
    if "user_id" not in session:
        return redirect("/login")
...
def events_signup():
    ...
    if event_name and full_name and email:
        ...
        if cursor.fetchone():
            ...
            conn.execute(
                "INSERT INTO event_signups (user_id, event_name, full_name, email) VALUES (?, ?, ?, ?)",
...
def subject(subject_id):
    ...
    subject_data = {
        ...
        "description": row[3], "course_name": row[4],
    ...
    """, (subject_id,))
    announcements = [
        {"id": r[0], "author": f"{r[1]} {r[2]}", "title": r[3], "content": r[4], "posted_at": r[5]}
        for r in cursor.fetchall()
    ]
    conn.close()

    return render_template("subject.html", subject=subject_data, comments=comments, announcements=announcements)
...
def add_comment(subject_id):
    ...
    content = request.form["content"]  # raw — intentional XSS surface
...
def profile():
    ...
    # VULNERABILITY: IDOR - user can change user_id parameter
    user_id = request.args.get('user_id', session['user_id'])
    
    # Always show user_id in URL to expose the vulnerability
    if 'user_id' not in request.args:
        return redirect(f"/profile?user_id={session['user_id']}")
    
    conn = get_db()
    cursor = conn.cursor()
    
    # Get user info for the requested user_id (no authorization check!)
    cursor.execute("SELECT first_name, last_name, email, role FROM users WHERE id = ?", (user_id,))
    user_data = cursor.fetchone()
    
    # Get courses for the requested user_id
    cursor.execute("""
    ...
    user = {"first_name": user_data[0], "last_name": user_data[1], "email": user_data[2], "role": user_data[3]} if user_data else None
...
def logout():
    ...
    return redirect("/login")
   # Vulnerability: Exposed Backup Files OWASP: A05:2021 - Security Misconfiguration
@app.route('/<path:filename>')
def serve_backup_files(filename):
    """
    VULNERABILITY: Serves backup files without validation
    Allows access to .bak, .old, .backup files
    """
    try:
...
if __name__ == '__main__':
    ...
    print(f"Flask running from: {BASE_DIR}")
